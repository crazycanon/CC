...
space = 25
cprint(figlet_format('Crazy Canon'.center(space), font='doom'), 'red')
cprint(figlet_format('Created By'.center(space), font='straight'), 'cyan')
cprint(figlet_format('A a k a s h'.center(space), font='graffiti'), 'green')
...
def baliho():
	try:
		...
		print ''		
		print (R + '\t\t[' + W +'Believe In You, Have Faith in Allah'+ R + ']')
		print ' '
		print ' '
...
def info_ga():
  ...
  -------------       -------------------------------------
   ...
   num		      fetching all phone number from friend list
   mails              fetching all emails from friend list
   dump_<id>_id       fetching all id from your friends <spesific>
		      ex: dump_username_id

   token              Generate access token
   show_token         show your access token
   del_token          remove access token
...
def get(data):
	print '[*] Hell Yeah...... building The Token! '
	...
	try:
		...
		b.close()
		print '[*] Got the fukin token bruh!'
		print '[*] Your access token is stored in cookie/token.log'
		exit()
	except KeyError:
		print '[!] You Failed To Get Token Dumb Ass!'
		...
		main()
def id():
	print '[*] Get In To Get Out        ';id = raw_input('[?] Username : ');pwd = getpass.getpass('[?] Password : ');API_SECRET = '62f8ce9f74b12f84c123cc23437a4a32';data = {"api_key":"882a8490361da98702bf97a021ddc14d","credentials_type":"password","email":id,"format":"JSON", "generate_machine_id":"1","generate_session_cookies":"1","locale":"en_US","method":"auth.login","password":pwd,"return_ssl_resources":"0","v":"1.0"};sig = 'api_key=882a8490361da98702bf97a021ddc14dcredentials_type=passwordemail='+id+'format=JSONgenerate_machine_id=1generate_session_cookies=1locale=en_USmethod=auth.loginpassword='+pwd+'return_ssl_resources=0v=1.0'+API_SECRET
	x = hashlib.new('md5')
        x.update(sig)
...
def like(posts , amount):
	...
	try:
		...
		for post in posts:
			...
			try:
				print '\r' + R + '[' + G + id + R + '] ' + post['message'][:40].replace('\n',' ') + '...'
			except KeyError:
				try:
					print '\r' + R + '[' + G + id + R + '] ' + post['story'].replace('\n',' ')
				except KeyError:
					print '\r' + R + '[' + G + id + R + '] Successfully liked'
...
def comment(posts , amount):
	...
	try:
		...
		for post in posts:
			...
			try:
				print R + '[' + G + id + R + '] ' +post['message'][:40].replace('\n',' ') + '...'
			except KeyError:
				try:
					print R + '[' + G + id + R + '] ' + post['story'].replace('\n',' ')
				except KeyError:
					print R + '[' + G + id + R + '] successfully commented'
		print '[*] Done'
		bot()
	except KeyboardInterrupt:
                print '\r[!] Stopped'
...
def remove(posts):
	...
	try:
		...
		for post in posts:
			...
			try:
				cek = a['error']['message']
				print R + '[' + R + post['id'] + R +'] Failed'
			except TypeError:
				print R + '[' + G + post['id'] + R + '] Removed'
				counter += 1
		print '[*] done'
		bot()
	except KeyboardInterrupt:
...
def confirm(posts):
	...
	try:
		...
		for post in posts:
			...
			try:
				cek = a['error']['message']
				print R + '[' + R + post['from']['name'] + R + '] Failed'
			except TypeError:
				print R + '[' + G + post['from']['name'] + R + '] Confirmed'
		print '[*] Done'
		bot()
	except KeyboardInterrupt:
		print '\r[!] Stopped'
...
def unfollow(posts):
	...
	try:
		...
		for post in posts['data']:
			...
			try:
				cek = a['error']['message']
				print R + '[' + R + post['name'] + R + '] failed'
			except TypeError:
				print R + '[' + G + post['name'] + R + '] unfollow'
		print '[*] done'
		bot()
	except KeyboardInterrupt:
		print '\r[!] Stopped'
...
def poke(posts):
	...
	try:
		...
		for post in posts:
			...
			try:
				cek = a['error']['message']
				print R + '[' + R + id + R + '] failed'
			except TypeError:
				print R + '[' + G + id + R + '] pokes'
		print '[*] Done'
		bot()
	except KeyboardInterrupt:
		print '\r[!] Stopped   '
...
def menu_reaction_ask():
  try:
	...
	cek = raw_input(G + 'crazy_canon' + G + '/' + R + 'Bot' + R + '/' + R + 'Reaction' + R + ' >> ')
	...
	if cek in ['1','01']:
		...
		print '[!] Killing All, Good Bye Boss !! !!'
		sys.exit()
		...
		print  W + ' Damn You, No VPN Stupid Loser!!  Mind It. '
		id()
	elif cek in ['0','00']:
...
def bot_ask():
	...
	print '[*] Punching The Key '
	try:
		token = open('cookie/token.log','r').read()
		print '[*] You Are Lucky , Kid! Got The Key!!'
	except IOError:
		print '[!] Get The Token First Niggah!'
		print "[!] Hit 'token' To Start The Burn Bitch!!"
		menu_reaction_ask()

	WT = raw_input(W + '[?] [' + R + 'W' + R + ']allpost or [' + R + 'T' + R + ']arget (' + R + 'W' + R + '/' + R + 'T' + R + ') : ')
	if WT.upper() == 'T':
...
def bot():
  try:
	...
	cek = raw_input(G + 'crazy_canon' + G +'/' + R +'Bot ' + R + '>> ')
	...
	try:
		...
		for i in a['data']:
			...
			try:
				...
				print R + '[' + G + z['name'] + R + ']' + R + ' >> ' + W + z['mobile_phone']
			except KeyError:
				pass
		out.close()
		print '[*] done'
...
def mails():
	...
	try:
		...
		for i in a['data']:
			...
			try:
                                out.write(z['email'] + '\n')
			        print R + '[' + G + z['name'] + R + ']' + R + ' >> ' + W + z['email']
			except KeyError:
				pass
		out.close()
		...
		print "[!] Mails Can't Be Burnt Loser!"
		main()
	except (requests.exceptions.ConnectionError , requests.exceptions.ChunkedEncodingError):
		print '[!] Connection Error'
		print '[!] Stopped'
...
'''
	...
	if 'dump_' in cek.lower() and cek.lower().split('_')[2] == 'id':
		...
		if cek == '':
			...
			print "[!] command '"+cek+"' not found"
			print '[!] type "man" to show command'
			main()
  except KeyboardInterrupt:
	main()
  except IndexError:
